...
AudioEye's rules are based on specific criteria we expect are present in your site or code. These are defined by WCAG guidelines (currently WCAG 2.2, levels A-AAA) plus best practices we have developed over years of advising clients how to avoid legal claims caused by accessibility errors.
...
Each individual test in the automated test package is called a rule (RuleCode). There are 
...
Not all rules apply to individual html components, and similarly some issues can only be detected
during a full page scan. The following rules will not work when using the `--component` flag

JONATHAN TO ADD LIST

### Rules Requiring Browser Context
Certain rules require the rendered style of the page to properly determine if there is an accessibility issue or not. The following rules will not work if you are using the CLI, Cypress, or Jest packages in a non-browser context. 

JONATHAN TO ADD LIST

## Check What Rules Run
To see what tests are being run use the `--print-test-list` flag on the CLI when running a
scan.
